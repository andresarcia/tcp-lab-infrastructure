...
void WimaxBackoffTimer::handle(Event *e)
{
  ...
  ///A Paltrinieri Debug
  //apaltrinieri::logger::getInstance()->write<<"CONTENTION REQ Transmiting at "<<NOW<<std::endl<<std::flush;
  ///
...
BwRequest::BwRequest (ContentionSlot *s, Packet *p, int cid, int len) : ContentionRequest (s,p)
{
  ...
  ///A Paltrinieri Debug
  //apaltrinieri::logger::getInstance()->write<<"BW REQ contention at "<<NOW<<" MAC "<<mac_->addr()<<std::endl<<std::flush;
  ///
...
void BwRequest::expire ()
{
  ...
  ///A Paltrinieri Debug
  //apaltrinieri::logger::getInstance()->write<<NOW<<std::endl<<std::flush;
  ((Mac802_16SS*)mac_)->increaseT16ExpirationCounter();
  ///
