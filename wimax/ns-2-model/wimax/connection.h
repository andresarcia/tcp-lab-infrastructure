...
class Connection {
 public:
  ...
  ////////    A Paltrinieri
  /**
  * Return queue size in bytes of the first X packets in the queue
  * @return The queue size in bytes of the first X packets
  */
  int queueFirstXByteLength (int x);
  ////////////////////////////////
 ...
 private:
  ...
  int newQueuedPacketSizeBytes;	  //Store the size in bytes of the new packets queued in the queue from de last call to queueByteLength

 public:
  int getNewQueuePacketInBytes() {
	  int temp = newQueuedPacketSizeBytes;
