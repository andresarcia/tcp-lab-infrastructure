...
Mac802_16BS::Mac802_16BS() : Mac802_16 (), cl_head_(0), cl_tail_(0), ctrlagent_(0)
{
  ...
  //A Paltrinieri
  pBWPerceptionPolicy = NULL;
  bind ("collision_counter_", &cCollisionCounter );
  bind ("bwreq_collision_counter_", &cBWReqCollisionCounter);
  bind ("bwreq_loss_counter_", &cTotalBWReqFailCounter);
  ////
...
Mac802_16BS::~Mac802_16BS() {
	if (pBWPerceptionPolicy)
		delete pBWPerceptionPolicy;
}
////
...
int Mac802_16BS::command(int argc, const char*const* argv)
{
	//A Paltrinieri
	if (argc == 3) {
	    if (strcmp(argv[1], "set-bw-perception-policy") == 0) {

	    	PerceptionManagement::BWPerceptionManagementPolicy* newPolicy;

	    	if (strcmp(argv[2], "RPG") == 0)
	    		newPolicy = new PerceptionManagement::RPGPolicy();
	    	else if (strcmp(argv[2], "DPG") == 0)
	    		newPolicy = new PerceptionManagement::DPGPolicy();
	    	else if (strcmp(argv[2], "iDDA") == 0)
	    		newPolicy = new PerceptionManagement::iDDAPolicy();
	    	else if (strcmp(argv[2], "dDDA") == 0)
	    		newPolicy = new PerceptionManagement::dDDAPolicy();
	    	else
	    	   return TCL_ERROR;

	    	if (pBWPerceptionPolicy)
	    			delete pBWPerceptionPolicy;
	    	pBWPerceptionPolicy = newPolicy;
	    	pBWPerceptionPolicy->setConnectionManager(connectionManager_);
	    	return TCL_OK;
	    }
	  }
	////
...
void Mac802_16BS::init ()
{
  ...
  //A Paltrinieri
  //Default Perception Policy
  if (!pBWPerceptionPolicy) {
	  pBWPerceptionPolicy  = new PerceptionManagement::dDDAPolicy();
	  pBWPerceptionPolicy->setConnectionManager(connectionManager_);
  }
  ///////
}
...
void Mac802_16BS::sendUp (Packet *p)
{
  ...
  if (pktRx_ !=NULL) {
    ...
    if(pktRx_->txinfo_.RxPr / p->txinfo_.RxPr >= p->txinfo_.CPThresh) {
      ...
      if (ch->ptype()==PT_MAC && HDR_MAC802_16(pktRx_)->header.ht != 0)
    	  cTotalBWReqFailCounter++;
    }
    return;
  }
  assert (pktRx_==NULL);
...
void Mac802_16BS::receive ()
{
  ...
  ///A Paltrinieri Debug
  //apaltrinieri::logger::getInstance()->write<<"At "<<NOW<<" in MAC "<<index_<<" packet received Type:"<<packet_info.name(ch->ptype());
  ///
  ...
  if (ch->ptype()==PT_MAC) {
	  if (HDR_MAC802_16(pktRx_)->header.ht == 0) {
		  ...
		  printf ("bwreq\n");
		  ///A Paltrinieri Debug
		  //apaltrinieri::logger::getInstance()->write<<" MAC TYPE: BW REQ";
		  ///
	  }
  } else {
    printf ("\n");
  ...
  ///A Paltrinieri Debug
  //apaltrinieri::logger::getInstance()->write<<std::endl<<std::flush;
  ///
#endif
  ...
  if (header.ht == 0) {
	//Is not a BW Req, Therefore Decrease perception BW needs size
	assert (pBWPerceptionPolicy);
	pBWPerceptionPolicy->updateConnectionPerception(con->get_cid(),ch->size()*-1);
  }
  ////
...
//A Paltrinieri
PerceptionManagement::BWPerceptionManagementPolicy* Mac802_16BS::getBWPerceptionPolicy () {
	assert (pBWPerceptionPolicy);
	return pBWPerceptionPolicy;
};
///
...
void Mac802_16BS::process_bw_req (Packet *p)
{
	...
	assert (pBWPerceptionPolicy);
	pBWPerceptionPolicy->newBWREQReceived(*((bw_req_header_t*)&header));

  /*bw_req_header_t *req;
  req = (bw_req_header_t *)&bwReq;
  ...
  if ( (req->type & 0x7) == 0x001) {
	...
	///A Paltrinieri Debug
	//apaltrinieri::logger::getInstance()->write<<"AGGREGATED BW REQ - SIZE "<<req->br<<std::endl<<std::flush;
	///
	//aggregate
