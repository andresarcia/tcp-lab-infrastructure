...
# calculates the Jain's index of fairness for a set of TCP flows
proc get-fairness-index { tcp } {
  ...
  if { $length == 0 } {
      error "Error: empty TCP flow list to in get-fairness-index";
  }

  for { set i 0 } { $i < $length } { incr i } {
      set thput [[lindex $tcp $i] get-throughput-bps]
      set sum_x [expr $sum_x + $thput]
      set sum_x_sq [expr $sum_x_sq + $thput*$thput]
  }

  set sum_x [ expr $sum_x*$sum_x ]
 
 
 if {$length*$sum_x_sq ==0} {
  	return 0;
  } 		
  return [ expr $sum_x/($length*$sum_x_sq) ]
} 
