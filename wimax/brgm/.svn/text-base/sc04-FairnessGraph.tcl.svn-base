...
proc finish {} {
    global ns tcp_list tf tcpTraceFairnessFile cmd_line_opt

    $ns flush-trace

    close $tf
   
    close $tcpTraceFairnessFile
...
#Fairness Graph
proc traceFairness {file} {
	global ns tcp_list
	set time 0.01
	set now [$ns now]
	puts [llength $tcp_list]
	set fairness [expr [get-fairness-index $tcp_list]]
	puts $file "$now $fairness"
	$ns at [expr $now+$time] "traceFairness $file"
}

#Open TCP Throughput Trace File
set tcpTraceFairnessFile [open [concat $cmd_line_opt(filename) {FairnessGraph}] w]
$ns at 0.0 "traceFairness $tcpTraceFairnessFile"
