...
[$bs_node set mac_(0)] set-bw-perception-policy RPG
...
proc finish {} {
    ...
    global ns tcp_list tf tcpAggregatedThroughputFile cmd_line_opt wl_nodes bs_node
    ...
    puts $tcpAggregatedThroughputFile "$cmd_line_opt(number_of_ss) $aggregatedThroughput"
   	...
   	puts "SS 1 T16: [[$wl_nodes(1) set mac_(0)] set t16_expiration_counter_]"
    puts "SS 1 BW-REQ Counter: [[$wl_nodes(1) set mac_(0)] set bwreq_sent_counter_]"
    
	puts "BW Req losses: [[$bs_node set mac_(0)] set bwreq_loss_counter_]"
    puts "BW Req Collisions: [[$bs_node set mac_(0)] set bwreq_collision_counter_]"
    puts "Collisions: [[$bs_node set mac_(0)] set collision_counter_]"
    
    puts [format "T16 Exp Rate: %f " [expr [get-mean-T16-exp-rate wl_nodes]] ]
    puts [format "BWReq Collision Rate: %f " [expr [[$bs_node set mac_(0)] get-bwreq-collision-rate]] ]
    puts [format "P: %f " [expr [get-bwreq-collision-probability $bs_node wl_nodes]] ]
